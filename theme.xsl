...
&lt;![endif]</xsl:comment><xsl:comment>  &lt;link rel="stylesheet" href="css/plone.css" type="text/css" &gt;
  ...
  <div id="page" class="page page-startseite page-118269"><xsl:text>
        ...
        Funktionen</h6><xsl:text>
          ...
          </xsl:text><xsl:apply-templates select="//*[@id = 'p-views']//li"/><xsl:text>
      ...
      </xsl:text><xsl:apply-templates select="//*[@id = 'p-personal']/node()"/><xsl:text>
      </xsl:text><div class="g-clear"/><xsl:text>
    </xsl:text></div><xsl:text>
    </xsl:text><hr class="g-hidden"/><div id="main" class="main"><xsl:text>
      </xsl:text><div id="mainNav" class="mod-mainnav"><xsl:text>
        </xsl:text><h6 class="g-area-heading area-heading">
        Hauptnavigation<xsl:apply-templates select="//*[@id = 'p-navigation']/node()"/><xsl:apply-templates select="//*[@id = 'p-tb']/node()"/></h6><xsl:text>

        </xsl:text><span id="LMUnav"/><xsl:text>

      </xsl:text></div><xsl:text>
      </xsl:text><hr class="g-hidden"/><div id="contentcontainer" class="contentcontainer contentcontainer-l g-grid-735"><xsl:text>
        </xsl:text><h6 class="g-area-heading area-heading">Inhaltsbereich</h6><xsl:text>
        </xsl:text><xsl:comment> Liste f&amp;#252;r Top-Thema ermitteln </xsl:comment><xsl:text>
        </xsl:text><xsl:comment> Liste f&amp;#252;r Meldungen ermitteln </xsl:comment><xsl:text>

        </xsl:text><div id="content" class="content content-start content-start-minibild2 user-html"><xsl:apply-templates select="//*[@id = 'content']/node()"/></div><xsl:text>

        </xsl:text><div id="cFn2" class="mod mod-cfn mod-cfn-2"><xsl:text>
          </xsl:text><hr class="g-hidden"/><h6 class="g-area-heading area-heading">Funktionsleiste</h6><xsl:text>

          </xsl:text><ul class="m-col-1 m-buttons"><li class="m-button m-button-print"><a href="#top" onclick="print();" class="m-link m-link-print" title="drucken">drucken</a></li><xsl:text>
          </xsl:text></ul><ul class="m-col-2 m-buttons"><li class="m-button m-button-top"><a href="#top" class="m-link m-link-top" title="nach oben">nach oben</a></li><xsl:text>
          </xsl:text></ul><div class="g-clear"/><xsl:text>
        </xsl:text></div><xsl:text>
      </xsl:text></div><xsl:text>
      </xsl:text><hr class="g-hidden"/><xsl:text>

      </xsl:text><div class="g-clear"/><xsl:text>
    </xsl:text></div><xsl:text>
    </xsl:text><hr class="g-hidden"/><div id="footer" class="mod-footer"><xsl:text>
